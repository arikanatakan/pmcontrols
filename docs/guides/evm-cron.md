...
Any indicator name in `r.stats` can be a threshold key; the alert fires
when the value falls below it. `r.alerts` is a tuple of structured `Alert`
records (`indicator`, `value`, `threshold`, `note`), so a downstream
system can route them rather than parse text.
